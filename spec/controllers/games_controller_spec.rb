...
RSpec.describe GamesController, type: :controller do
  ...
  describe "POST create" do
    ...
    it "returns a JSON board with statuses only" do
      ...
      expect(board.none? { |cell| cell['content'] }).to be true
    end
  ...
  describe "GET show" do
    ...
    context "game does not exist" do
      ...
      it "returns 404 not found response code"

      it "returns error message as JSON response body" do
        expect(json['error']).to match("Couldn't find Game with 'id'=#{game.id}")
      end
    end

    context "other error occurs" do
      it "returns 500 server error response code"
  ...
  describe "PATCH fire" do
    ...
    context "cell has not been fired upon" do
      it "returns the updated cell status as JSON response body" do
        expect(json['status']).to eq("miss").or eq("hit")
      end
    end
